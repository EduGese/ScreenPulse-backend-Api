...
import { FavoriteResponse, FavoritesListWithMetadata, MediaItemDocument, MediaItemInput, MediaItemWithMetaData } from "../../interfaces/favorites.interface";
...
*/
  ...
  * */
  async deleteFavorite(movieId: string, userId: string): Promise<void> {
    ...
    favorite.user = favorite.user.filter((u: { toString: () => string; }) => u.toString() !== userId);
    await favorite.save();
